...
### [20:45] Learnings
**Learning (Technical):** JSON hygiene principle: Omit keys when values are absent/None rather than using null or magic values like -1. Absence is a clearer semantic signal than ambiguous null (which could mean "unknown," "not set," or "intentionally empty"). Pattern: only include key in dict if it carries meaningful information.

### [20:45] Learnings
**Learning (Process):** "Self-healing with confession" pattern: Systems should document their failures and repairs in audit logs rather than hiding them. Repair logs should include trigger context (manual vs automated, batch limits) and before/after metrics. This creates accountability and aids debugging.

### [20:45] Learnings
**Learning (Process):** Crash drill checklist approach: After implementing recovery features, create explicit pass/fail verification steps - confirm tool registration, start operation, kill mid-flight, restart, verify recovery state. Proves resilience under real operational chaos.

### [20:46] Task Completed
**Task:** Finalized repair logging design for duro_prune_orphans
**Outcome:** Decided on Option 2 (omit when absent) for max_delete in before_metrics. Established consistent contract: if key exists, it's meaningful; if absent, wasn't relevant. Applied same hygiene to missing_ids.

### [20:46] Task Completed
**Task:** Created MCP restart crash drill checklist
**Outcome:** 7-step verification checklist covering: tool registration, reembed start, progress evidence, mid-flight kill, restart, recovery verification, and prune audit trail. Ready for next MCP restart testing.

### [21:04] Task Completed
**Task:** Create crash_drill_verify skill for Duro MCP resilience testing
**Outcome:** Created ops/crash_drill_verify.py skill that automates crash drill verification: import check, embedding availability, mini reembed (5 artifacts), repair log check, stuck repairs check, SQLite integrity, index health, semantic search test. All 8 checks pass. Added to skills index.json.

### [21:09] Task Completed
**Task:** Fix hookify hang for MCP tools
**Outcome:** Added fast-path optimization to PreToolUse and PostToolUse hooks in hookify plugin. MCP tools (tool_name.startswith('mcp__')) now skip rule evaluation entirely since hookify rules only target Bash and file events. PostToolUse still performs S2 secret scanning for MCP tool output. This should eliminate the hang when calling duro_reembed and other MCP tools.

### [21:09] Learnings
**Learning (Technical):** Hookify PreToolUse/PostToolUse hooks can cause hangs for MCP tools when rule loading or evaluation takes too long. The fix is to add a fast-path early exit for MCP tools (tool_name.startswith('mcp__')) since hookify rules only target 'bash' and 'file' events - MCP tools have their own permission system. Location: ~/.claude/plugins/cache/claude-plugins-official/hookify/*/hooks/pretooluse.py and posttooluse.py

### [21:14] Task Completed
**Task:** Fix hookify MCP tool hang - apply fix to marketplace hooks
**Outcome:** Applied MCP fast-path fix to ~/.claude/plugins/marketplaces/claude-plugins-official/plugins/hookify/hooks/pretooluse.py and posttooluse.py. Previous fix was only applied to cache directory which wasn't being used. Fix adds early exit for tool_name.startswith('mcp__') to skip unnecessary rule evaluation.

### [21:20] Task Completed
**Task:** Create duro_bash_ops skill to bypass MCP hook hang
**Outcome:** Created ~/.agent/skills/ops/duro_bash_ops.py skill that runs Duro operations via direct Python import, completely bypassing MCP tool calls and hookify PreToolUse hooks. Commands: crash-drill [--full], health, reembed N, prune [--apply], status. All commands tested and working. Added to skills index.

### [21:20] Learnings
**Learning (Technical):** When hookify PreToolUse hooks cause MCP tools to hang indefinitely (despite 10s timeout), the workaround is to bypass MCP entirely by running operations via Bash(python script.py). Created duro_bash_ops.py skill that imports duro-mcp modules directly and runs operations without going through MCP server. This avoids the hook execution path completely. Root cause appears to be Claude Code's hook execution mechanism for MCP tools, not the hook code itself - the hook works when tested directly but hangs when Claude Code invokes it for MCP tools.

### [21:23] Learnings
**Learning (Process):** The duro_bash_ops skill successfully bypasses MCP hook hangs by running operations via direct Python imports instead of MCP tool calls. When hookify PreToolUse hooks hang indefinitely on MCP tools (especially duro_reembed), use: python ~/.agent/skills/ops/duro_bash_ops.py crash-drill. This completes in ~2 seconds vs hanging for 12+ minutes.

### [21:23] Task Completed
**Task:** Session: Fix MCP hook hang with bash bypass skill
**Outcome:** Created duro_bash_ops.py skill after multiple failed attempts to fix hookify PreToolUse hook hang. Applied fix to both cache and marketplace hook files but Claude Code still hung. Solution: bypass MCP entirely via direct Python imports. Skill provides: crash-drill, health, reembed, prune, status commands. All work instantly without hooks.

### [22:00] Task Completed
**Task:** Create MSJ data model v1
**Outcome:** Created comprehensive data model with 11 core entities: Swimmer, Parent, Meet, Event, RaceLog, MeetDayLog, ParentObservation, TrainingSession, PersonalBest, Goal, Equipment. Also includes computed types for StrokeRanking, SplitAnalysis, PerformanceCorrelation. Stored in ~/Desktop/BUILD/msj/data-model.md
