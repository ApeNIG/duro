...
def generate_id(artifact_type: str) -> str:
    ...
    prefix_map = {
        ...
        "incident_rca": "inc",
        "recent_change": "chg",
        "design_reference": "dref",
        "checklist_template": "chk",
        "decision_validation": "dval"
    }
...
class ArtifactStore:
    ...
    # Debug Gate: Banned prevention phrases (handwaving)
    BANNED_PREVENTION_PHRASES = [
        "be careful",
        "pay attention",
        "double check",
        "double-check",
    ...
    def _validate_debug_gate(
        self,
        repro_steps: Optional[list[str]],
        first_bad_boundary: Optional[str],
        prevention: Optional[str],
        recent_change_scan: Optional[dict],
        tags: Optional[list[str]]
    ) -> tuple[bool, list[str]]:
        """
        Validate debug gate passes. Returns (passed, list_of_failures).

        Pass 1: Repro - repro_steps not empty, at least 2 steps
        Pass 2: Boundary - first_bad_boundary names a boundary surface
        Pass 3: Causality - recent_change_scan performed, linked or cleared
        Prevention: Must be actionable (verb + artifact), no banned phrases
        """
        failures = []

        # Pass 1: Repro
        if not repro_steps or len(repro_steps) < 2:
            failures.append("Pass 1 (Repro): Missing repro_steps or fewer than 2 steps")

        # Pass 2: Boundary
        if not first_bad_boundary or len(first_bad_boundary.strip()) < 5:
            failures.append("Pass 2 (Boundary): Missing first_bad_boundary (must name boundary surface: config path, env var, API response, DB read/write, etc.)")

        # Pass 3: Causality (48-hour scan)
        if not recent_change_scan:
            # Auto-run if not provided (will be done in store_incident)
            failures.append("Pass 3 (Causality): Missing recent_change_scan (must run 48-hour query)")
        else:
            results = recent_change_scan.get("results", [])
            linked = recent_change_scan.get("linked", [])
            cleared_reason = recent_change_scan.get("cleared_reason", "")

            # Must either link a change OR provide cleared_reason
            if results and not linked and not cleared_reason:
                failures.append(f"Pass 3 (Causality): Found {len(results)} recent changes but none linked. Either link relevant changes or provide cleared_reason")
            elif not results and not cleared_reason:
                failures.append("Pass 3 (Causality): No recent changes found. Provide cleared_reason to confirm no infra/config changes are related")

        # Prevention check
        if not prevention or len(prevention.strip()) < 10:
            failures.append("Prevention: Missing or too short (must be actionable: verb + artifact/guardrail)")
        else:
            prevention_lower = prevention.lower()
            for banned in self.BANNED_PREVENTION_PHRASES:
                if banned in prevention_lower:
                    failures.append(f"Prevention: Contains banned phrase '{banned}'. Must be actionable (e.g., 'Add startup log', 'Assert X exists', 'Add smoke test')")
                    break
    ...
    def store_incident(
        ...
        first_bad_boundary: Optional[str] = None,
        why_not_caught: Optional[str] = None,
        prevention: Optional[str] = None,
        related_recent_changes: Optional[list[str]] = None,
        # Debug Gate fields
        repro_steps: Optional[list[str]] = None,
        recent_change_scan: Optional[dict] = None,
        override: bool = False,
        ...
        """
        Store an incident RCA (Root Cause Analysis) artifact.

        DEBUG GATE ENFORCEMENT:
        This method enforces the debug gate - you cannot close an incident unless:
        1. Pass 1 (Repro): repro_steps has at least 2 steps
        2. Pass 2 (Boundary): first_bad_boundary names a boundary surface
        3. Pass 3 (Causality): recent_change_scan performed, changes linked or cleared
        4. Prevention: Must be actionable (no handwaving like "be careful")

        Use override=True with override_reason to bypass (creates waiver trail).

        Returns (success, artifact_id_or_error, file_path_or_gate_failures).
        """
        ...
        # Auto-run 48-hour scan if not provided
        if not recent_change_scan:
            # Infer risk_tags from incident tags
            ...
            if not inferred_risk_tags:
                inferred_risk_tags = ["config", "env", "paths"]  # Default suspects

            results = self.query_recent_changes(hours=48, risk_tags=inferred_risk_tags, limit=20)
            recent_change_scan = {
                "hours": 48,
                "risk_tags": inferred_risk_tags,
                "results": [r["id"] for r in results],
                "linked": related_recent_changes or [],
                "cleared_reason": None
            }

        # Sync linked from related_recent_changes if provided
        if related_recent_changes:
            current_linked = recent_change_scan.get("linked", [])
            merged_linked = list(set(current_linked + related_recent_changes))
            recent_change_scan["linked"] = merged_linked

        # Validate debug gate
        gate_passed, failures = self._validate_debug_gate(
            repro_steps=repro_steps,
            first_bad_boundary=first_bad_boundary,
            prevention=prevention,
            recent_change_scan=recent_change_scan,
            tags=tags
        )

        # Gate enforcement
        if not gate_passed and not override:
            failure_text = "\n".join(f"• {f}" for f in failures)
            return (False, "GATE_BLOCKED", f"Debug gate blocked. Missing:\n{failure_text}\n\nUse override=True with override_reason to bypass.")

        # Override validation
        if override and not override_reason:
            return (False, "OVERRIDE_REQUIRES_REASON", "override=True requires override_reason (e.g., 'prod down, mitigated first')")

        # If overriding, add waiver tag
        if override:
            if "waived_gate" not in tags:
                tags.append("waived_gate")

        artifact_id = generate_id("incident_rca")

        # Determine status
        status = "fixed" if gate_passed else "draft"
        if override:
            status = "fixed"  # Override allows closing
        ...
        artifact = {
            ...
            "data": {
                ...
                "fix": fix,
                "prevention": prevention,
                "related_recent_changes": related_recent_changes or [],
    ...
    def store_recent_change(
        self,
        scope: str,
        change: str,
        why: Optional[str] = None,
        risk_tags: Optional[list[str]] = None,
        quick_checks: Optional[list[str]] = None,
        ...
        """
        Store a recent change artifact for the change ledger.
        Returns (success, artifact_id, file_path).
        """
        artifact_id = generate_id("recent_change")

        # Validate risk_tags
        valid_risk_tags = ["config", "db", "paths", "sync", "deploy", "auth", "caching", "env", "permissions", "network", "state", "api", "schema"]
        filtered_risk_tags = [t for t in (risk_tags or []) if t in valid_risk_tags]
    ...
    def query_recent_changes(
        ...
        """
        Query recent changes within a time window, optionally filtered by risk tags.
        This is the core of the "48-hour rule" for debugging.
        """
        ...
        for entry in entries:
            # Load full artifact to get data
            ...
            # Filter by risk_tags if specified
            if risk_tags:
                artifact_risk_tags = data.get("risk_tags", [])
                if not any(t in artifact_risk_tags for t in risk_tags):
                    continue
            ...
            results.append({
                ...
                "why": data.get("why"),
                "quick_checks": data.get("quick_checks", [])
            })
    ...
    def store_design_reference(
        ...
        stealable_rules: Optional[list[str]] = None,
        ...
        """
        Store a design reference for the taste library.
        Returns (success, artifact_id, file_path).
        """
        artifact_id = generate_id("design_reference")
        ...
        artifact = {
            ...
            "data": {
                ...
                "stealable_rules": stealable_rules or [],
    ...
    def store_checklist_template(
        ...
        items should be list of dicts with: text, category (optional), required (optional, default True)
        """
        artifact_id = generate_id("checklist_template")

        # Normalize items
        normalized_items = []
        for item in items:
            if isinstance(item, str):
                normalized_items.append({"text": item, "required": True})
    ...
    def validate_decision(
        ...
        Updates the decision's current truth AND creates an append-only
        validation event artifact for full history.
        ...
        Args:
            ...
            next_action: What to do next based on this validation
            confidence_delta: Override automatic confidence adjustment

        Returns (success, message, validation_id).
        """
        # Get decision
        ...
        if artifact.get("type") != "decision":
            return False, f"Artifact '{decision_id}' is not a decision", None

        # Validate status
        valid_statuses = ["validated", "reversed", "superseded"]
        if status not in valid_statuses:
            return False, f"Invalid status '{status}'. Must be one of: {valid_statuses}", None
        ...
        if confidence_delta is not None:
            # Use provided delta
            actual_delta = confidence_delta
            new_conf = max(0.05, min(0.99, current_conf + confidence_delta))
        else:
        ...
        # Create append-only validation event
        validation_id = self._store_validation_event(
            ...
            confidence_delta=round(actual_delta, 2),
            confidence_after=round(new_conf, 2),
            episode_id=episode_id,
            notes=notes
        )

        return True, "Decision validated", validation_id

    def _store_validation_event(
        self,
        decision_id: str,
        status: str,
        result: Optional[str] = None,
        ...
        """
        Create an append-only validation event artifact.

        This provides full history of all validations for a decision.
        """
        artifact_id = generate_id("decision_validation")
        now = utc_now_iso()
        ...
        if expected_outcome:
            validation_data["expected_outcome"] = expected_outcome
        if actual_outcome:
            validation_data["actual_outcome"] = actual_outcome
        if next_action:
            validation_data["next_action"] = next_action
        if confidence_delta is not None:
            validation_data["confidence_delta"] = confidence_delta
        if confidence_after is not None:
            validation_data["confidence_after"] = confidence_after
        if episode_id:
            validation_data["episode_id"] = episode_id
        if notes:
            validation_data["notes"] = notes
        ...
        artifact = {
            ...
            "tags": ["decision-closure"],
            "source": {
                "workflow": "validate_decision",
                "run_id": None,
                "tool_trace_path": None
            },
        ...
        success, stored_id, path = self._store_artifact(artifact)
        if success:
            # Add relation: validation -> decision
    ...
    def get_validation_history(self, decision_id: str) -> list[dict]:
        """
        Get all validation events for a decision.

        Returns list of validation artifacts, oldest first.
        """
        validations = []

        # Query index for decision_validation artifacts
        all_validations = self.index.query(artifact_type="decision_validation")
        for v in all_validations:
            artifact = self.get_artifact(v["id"])
            if artifact and artifact.get("data", {}).get("decision_id") == decision_id:
                validations.append(artifact)

        # Sort by created_at (oldest first for chronological history)
        validations.sort(key=lambda x: x.get("created_at", ""))
        return validations

    def get_decision_review_context(
        self,
        decision_id: str,
        hours_recent_changes: int = 48,
        risk_tags: Optional[list[str]] = None
    ) -> dict:
        """
        Gather all context needed for a decision review.

        Returns a dict with:
        - decision: core decision data
        - validation_history: list of past validations
        - linked_episodes: top 3 most recent
        - linked_incidents: top 3 most recent
        - recent_changes: 48-hour scan results
        - error: if decision not found
        """
        # Get decision
        artifact = self.get_artifact(decision_id)
        if not artifact:
        ...
        if artifact.get("type") != "decision":
            return {"error": f"Artifact '{decision_id}' is not a decision"}
        ...
        decision_core = {
            ...
            "outcome_status": outcome.get("status", "unverified"),
            "outcome_result": outcome.get("result"),
            "outcome_confidence": outcome.get("confidence", 0.5),
            "verified_at": outcome.get("verified_at")
        }

        # Get validation history (last 3 or all if < 3)
        all_validations = self.get_validation_history(decision_id)
        # Take last 3 (most recent)
        validation_history = all_validations[-3:] if len(all_validations) > 3 else all_validations

        # Get linked episodes
        episodes_used = data.get("episodes_used", [])
        linked_episodes = []
        for ep_id in episodes_used[-3:]:  # Last 3 most recent
            ep = self.get_artifact(ep_id)
            if ep:
                ep_data = ep.get("data", {})
                linked_episodes.append({
                    "id": ep_id,
                    "goal": ep_data.get("goal", "")[:80],
                    "result": ep_data.get("result"),
                    "created_at": ep.get("created_at", "")
                })
        linked_episodes_count = len(episodes_used)

        # Get linked incidents (search incidents that reference this decision)
        linked_incidents = []
        linked_incidents_count = 0
        all_incidents = self.index.query(artifact_type="incident_rca")
        for inc in all_incidents:
            inc_artifact = self.get_artifact(inc["id"])
            if inc_artifact:
                inc_data = inc_artifact.get("data", {})
                # Check if decision is mentioned in related_recent_changes or notes
                related = inc_data.get("related_recent_changes", [])
                fix = inc_data.get("fix", "") or ""
                prevention = inc_data.get("prevention", "") or ""
                if decision_id in str(related) or decision_id in fix or decision_id in prevention:
                    linked_incidents_count += 1
                    if len(linked_incidents) < 3:
                        linked_incidents.append({
                            "id": inc["id"],
                            "symptom": inc_data.get("symptom", "")[:60],
                            "severity": inc_data.get("severity", "medium"),
                            "created_at": inc_artifact.get("created_at", "")
                        })

        # Derive risk_tags from decision tags if not provided
        if not risk_tags:
            # Map common decision tags to risk tags
            tag_to_risk = {
                "config": "config",
                "database": "db",
                "db": "db",
            ...
            for tag in artifact.get("tags", []):
                tag_lower = tag.lower()
                if tag_lower in tag_to_risk:
                    risk_tags.append(tag_to_risk[tag_lower])

        # Query recent changes
        recent_changes = self.query_recent_changes(
            hours=hours_recent_changes,
    ...
    def list_unreviewed_decisions(
        ...
        """
        List decisions that need review (not validated, old enough to have outcomes).

        This is the surfacing mechanism for decision review workflow.

        Args:
            older_than_days: Only include decisions older than N days (default 14)
            exclude_tags: Tags to exclude (default: smoke-test, auto-outcome, generated)
            include_tags: Only include decisions with these tags (optional filter)
            limit: Max decisions to return (default 20)

        Returns:
            List of decision summaries sorted by (overdue first, then age):
            [
                {
                    "id": "decision_...",
                    "decision": "Use Redis for caching",
        ...
        cutoff_dt = utc_now() - timedelta(days=older_than_days)
        now = utc_now()

        # Query all decisions
        entries = self.index.query(artifact_type="decision", limit=500)

        # Query all incidents for cross-referencing
        incident_entries = self.index.query(artifact_type="incident_rca", limit=200)
        incidents_by_decision = {}  # decision_id -> count
        for inc_entry in incident_entries:
            inc = self.get_artifact(inc_entry["id"])
            if inc:
                related = inc.get("data", {}).get("related_recent_changes", [])
                # Also check tags for decision references
                inc_tags = inc.get("tags", [])
                for tag in inc_tags:
                    if tag.startswith("decision_"):
                        incidents_by_decision[tag] = incidents_by_decision.get(tag, 0) + 1

        unreviewed = []

        for entry in entries:
            # Parse created_at to datetime (safe comparison)
            ...
            # Check age against cutoff (datetime comparison, not string)
            if created_dt > cutoff_dt:
                continue  # Too recent

            # Check exclude_tags
            entry_tags = set(entry.get("tags", []))
            if entry_tags & set(exclude_tags):
                continue  # Has excluded tag

            # Check include_tags (if specified)
            if include_tags:
                if not (entry_tags & set(include_tags)):
                    continue  # Doesn't have required tag
            ...
            # Handle missing/None outcome or old string format
            if outcome is None or isinstance(outcome, str):
                outcome = {"status": "unverified", "confidence": 0.5, "verified_at": None}
            ...
            # - unverified always needs review
            # - validated/reversed might need re-review if old enough
            needs_review = (status == "unverified")

            # Calculate age in days
            age_days = round((now - created_dt).total_seconds() / 86400, 1)

            # Count linked episodes
            episodes_used = data.get("episodes_used", [])
            linked_episodes_count = len(episodes_used) if isinstance(episodes_used, list) else 0

            # Count linked incidents
            decision_id = artifact["id"]
            linked_incidents_count = incidents_by_decision.get(decision_id, 0)
            ...
            unreviewed.append({
                ...
                "linked_episodes_count": linked_episodes_count,
                "linked_incidents_count": linked_incidents_count,
                "last_validated_at": verified_at,
                "needs_review": needs_review
            })

        # Sort: needs_review first, then by age (oldest first)
        unreviewed.sort(key=lambda x: (
            0 if x.get("needs_review") else 1,  # needs_review first
            -(x.get("age_days") or 0)  # then oldest first (negative for DESC)
        ))
    ...
    def _store_artifact(self, artifact: dict[str, Any]) -> tuple[bool, str, str]:
        ...
        try:
            ...
            # Embedding failure shouldn't block artifact storage
            print(f"[WARN] Sync embedding failed: {e}", file=sys.stderr)
