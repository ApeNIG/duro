...
class ArtifactIndex:
    ...
    # Default timeout for SQLite busy lock (ms)
    ...
    def _connect(self) -> sqlite3.Connection:
        """Create a connection with busy_timeout and WAL mode for better concurrency."""
    ...
    def _init_db(self):
        ...
        with self._connect() as conn:
            ...
            # Repair audit log - tracks self-healing operations
            conn.execute("""
                CREATE TABLE IF NOT EXISTS repairs (
                    ...
                    canonical_mutations INTEGER DEFAULT 0,
                    code_version TEXT,
                    result TEXT NOT NULL DEFAULT 'pending',
                    error_blob TEXT
                )
            """)
            conn.execute("CREATE INDEX IF NOT EXISTS idx_repairs_type ON repairs(repair_type)")
            conn.execute("CREATE INDEX IF NOT EXISTS idx_repairs_started ON repairs(started_at)")
            conn.commit()
    ...
    def _extract_title(self, artifact_type: str, data: dict) -> str:
        ...
        if artifact_type == "fact":
            ...
            return symptom[:100] if symptom else "Untitled incident"
        elif artifact_type == "recent_change":
            change = data.get("change", "")
            scope = data.get("scope", "")
            prefix = f"[{scope}] " if scope else ""
            return f"{prefix}{change[:90]}" if change else "Untitled change"
        elif artifact_type == "checklist_template":
            name = data.get("name", "")
            return f"Checklist: {name}" if name else "Untitled checklist"
        elif artifact_type == "design_reference":
            product = data.get("product_name", "")
            pattern = data.get("pattern", "")
            return f"{product} - {pattern}"[:100] if product else "Untitled design ref"
        elif artifact_type == "decision_validation":
            decision_id = data.get("decision_id", "")
            status = data.get("status", "")
            return f"Validation: {decision_id} -> {status}"[:100]
        return "Unknown"
    ...
    def get_embedding_state(self, artifact_id: str) -> Optional[dict]:
        """
        Get the embedding state for an artifact.

        Returns:
            Dict with content_hash, embedded_at, model, or None if not embedded.
        """
        try:
            with self._connect() as conn:
                conn.row_factory = sqlite3.Row
                cursor = conn.execute(
                    "SELECT content_hash, embedded_at, model FROM embedding_state WHERE artifact_id = ?",
                    (artifact_id,)
                )
                row = cursor.fetchone()
                if row:
    ...
    def get_missing_embedding_ids(self, limit: int = 100) -> list[str]:
        """
        Get artifact IDs that are missing embeddings.

        Args:
            limit: Max IDs to return

        Returns:
            List of artifact IDs without embeddings
        """
        try:
            with self._connect() as conn:
                cursor = conn.execute("""
                    SELECT a.id FROM artifacts a
                    LEFT JOIN embedding_state e ON e.artifact_id = a.id
                    WHERE e.artifact_id IS NULL
                    LIMIT ?
                """, (limit,))
    ...
    def get_embedded_artifact_ids(self) -> list[str]:
        """
        Get all artifact IDs that have embeddings.

        Returns:
            List of artifact IDs with embeddings
        """
        try:
            with self._connect() as conn:
    ...
    def count_orphan_embeddings(self) -> int:
        """Count embeddings for artifacts that no longer exist."""
        try:
            with self._connect() as conn:
                count = conn.execute("""
                    SELECT COUNT(*) FROM embedding_state e
    ...
    def list_orphan_embeddings(self, limit: int = 25) -> list[str]:
        """List orphan embedding IDs (for diagnostics)."""
        try:
            with self._connect() as conn:
                rows = conn.execute("""
                    SELECT e.artifact_id FROM embedding_state e
                    LEFT JOIN artifacts a ON a.id = e.artifact_id
                    WHERE a.id IS NULL
                    LIMIT ?
                """, (limit,)).fetchall()
    ...
    def prune_orphan_embeddings(self, max_delete: int = None) -> dict:
        """
        Delete embeddings for artifacts that no longer exist.
        Deletes in chunks of 200 to avoid SQLite parameter limits.

        Args:
            max_delete: Maximum orphans to delete (None = unlimited).
                        Use for batched cleanup of large orphan counts.

        Returns dict with 'count', 'pruned_ids' (first 50), 'remaining'.
        """
        CHUNK_SIZE = 200

        try:
            with self._connect() as conn:
                # Get orphan IDs (with optional limit)
                query = """
                    SELECT e.artifact_id FROM embedding_state e
                    LEFT JOIN artifacts a ON a.id = e.artifact_id
                    WHERE a.id IS NULL
                ...
                if not orphans:
                    return {"count": 0, "pruned_ids": [], "remaining": 0}

                orphan_ids = [o[0] for o in orphans]
                has_vectors = self._has_vectors(conn)

                # Delete in chunks to avoid parameter limit (999)
                for i in range(0, len(orphan_ids), CHUNK_SIZE):
                    ...
                    if has_vectors:
                        conn.execute(f"""
                            DELETE FROM artifact_vectors
                            WHERE artifact_id IN ({placeholders})
                        """, chunk)

                conn.commit()

                # Check remaining (for batched deletes)
                remaining = self.count_orphan_embeddings()

                return {
                    "count": len(orphan_ids),
                    "pruned_ids": orphan_ids[:50],  # Return first 50 for logging
                    "remaining": remaining
                }
        except Exception as e:
            print(f"Prune orphan embeddings error: {e}")
            return {"count": 0, "pruned_ids": [], "remaining": -1, "error": str(e)}
    ...
    def start_repair(
        ...
        """
        Start logging a repair operation.

        Args:
            repair_type: Type of repair (fts_rebuild, embed_backfill, full_reindex)
            trigger: What triggered the repair (e.g., "embedding_coverage < 95%")
            before_metrics: Metrics snapshot before repair
            missing_ids: List of artifact IDs that were missing (if known)
            code_version: Git SHA or version string

        Returns:
            repair_id for completing the log later
        """
        try:
            with self._connect() as conn:
                cursor = conn.execute("""
                    INSERT INTO repairs (
                        repair_type, trigger, started_at, before_metrics,
                        missing_count, missing_ids, code_version, result
                    ) VALUES (?, ?, ?, ?, ?, ?, ?, 'in_progress')
                """, (
                    repair_type,
                    trigger,
                    ...
                    len(missing_ids) if missing_ids else 0,
                    json.dumps(missing_ids[:100]) if missing_ids else None,  # Cap at 100
                    code_version
                ))
                conn.commit()
                return cursor.lastrowid
        except Exception as e:
            print(f"[WARN] Failed to start repair log: {e}")
            return -1
    ...
    def complete_repair(
        ...
        """
        Complete a repair log entry.

        Args:
            repair_id: ID from start_repair
            after_metrics: Metrics snapshot after repair
            processed_count: How many items were processed
            failed_count: How many failed
            canonical_mutations: Must be 0 for valid repairs
            result: "success", "partial", or "failed"
            error_blob: Error details if failed
        ...
        try:
            with self._connect() as conn:
                ...
                cursor = conn.execute(
                    "SELECT started_at FROM repairs WHERE id = ?",
                    (repair_id,)
                )
                row = cursor.fetchone()
                if not row:
            ...
            print(f"[WARN] Failed to complete repair log: {e}")
            return False

    def get_recent_repairs(self, limit: int = 20) -> list[dict]:
    ...
    def cleanup_stuck_repairs(self, max_age_minutes: int = 10) -> int:
        """
        Mark stuck in_progress repairs as failed.

        Should be called on server startup or periodically.

        Args:
            max_age_minutes: Repairs in_progress longer than this are marked failed

        Returns:
            Number of repairs marked as failed
        """
        try:
            with self._connect() as conn:
                ...
                cursor = conn.execute("""
                    ...
                    SET result = 'failed',
                        ended_at = ?,
                        error_blob = 'Interrupted / server restart'
            ...
            print(f"[WARN] Failed to cleanup stuck repairs: {e}")
            return 0
