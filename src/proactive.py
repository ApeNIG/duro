...
class ProactiveRecall:
    ...
    def recall(
        ...
        ARCHITECTURE: Semantic-first, keywords boost (not gate).
        Always searches semantically. Keywords boost relevance but never block.
        ...
        Args:
            ...
            force: If True, always search even if hot path says no (legacy, now always searches)
        ...
        if include_types:
            artifact_types = include_types
        else:
            # Default to searching facts, decisions, and episodes
        ...
        if boost_tags and memories:
            for memory in memories:
                ...
                # Boost score if memory tags overlap with keyword-derived tags
                overlap = len(set(memory_tags) & set(boost_tags))
                if overlap > 0:
                    memory["search_score"] = memory.get("search_score", 0) + (overlap * 0.05)
            # Re-sort by boosted score
            memories.sort(key=lambda m: m.get("search_score", 0), reverse=True)

        # Step 5: Load full artifacts and format results
        results = []
        ...
        return RecallResult(
            triggered=True,
            reason=f"Semantic recall: {len(results)} memories (keywords boosted: {', '.join(boost_tags) if boost_tags else 'none'})",
            memories=results,
