...
ARTIFACT_ENVELOPE_SCHEMA = {
    ...
    "properties": {
        ...
        "type": {
            "type": "string",
            "enum": ["fact", "decision", "skill", "rule", "log", "incident_rca", "recent_change", "design_reference", "checklist_template"],
...
# Incident RCA artifact data schema (Root Cause Analysis)
# Includes Debug Gate fields for enforced debugging discipline
INCIDENT_RCA_DATA_SCHEMA = {
    "type": "object",
    ...
    "properties": {
        ...
        "first_bad_boundary": {
            "type": "string",
            "description": "First place where output becomes wrong (config path, env var, API response, DB read/write, service startup, etc.)"
        },
        "actual_cause": {
            "type": "string",
        ...
        "prevention": {
            "type": "string",
            "description": "How to prevent recurrence (must be actionable: verb + artifact/guardrail)"
        },
        ...
        # Debug Gate: Pass 1 - Repro
        "repro_steps": {
            "type": "array",
            "items": {"type": "string"},
            "description": "Steps to reproduce the issue (gate requires at least 2 steps)"
        },
        # Debug Gate: Pass 3 - 48-hour scan
        "recent_change_scan": {
            "type": "object",
            "properties": {
                "hours": {"type": "integer", "default": 48},
                "risk_tags": {"type": "array", "items": {"type": "string"}},
                "results": {"type": "array", "items": {"type": "string"}, "description": "Change IDs found"},
                "linked": {"type": "array", "items": {"type": "string"}, "description": "Change IDs actually linked"},
                "cleared_reason": {"type": "string", "description": "Required if no changes linked"}
            },
            "description": "48-hour recent change scan results"
        },
        # Debug Gate: Override mechanism
        "override": {
            "type": "boolean",
            "default": False,
            "description": "True if gate was bypassed"
        },
        "override_reason": {
            "type": "string",
            "description": "Required explanation when override=true"
        },
        "status": {
            "type": "string",
            "enum": ["draft", "open", "rooted", "fixed", "verified"],
            "default": "draft",
            "description": "draft=incomplete, open=investigating, rooted=cause found, fixed=applied, verified=prevention confirmed"
        },
        "severity": {
            "type": "string",
...
# Recent Change artifact data schema (Change Ledger)
RECENT_CHANGE_DATA_SCHEMA = {
    "type": "object",
    ...
    "properties": {
        ...
        "links": {
            ...
            "description": "References to commits, PRs, issues, docs"
        },
        "auto_captured": {
            "type": "boolean",
            "default": False,
            "description": "Whether this was auto-captured from a commit"
        }
    }
}

# Design Reference artifact data schema (Taste Library)
DESIGN_REFERENCE_DATA_SCHEMA = {
    "type": "object",
...
# Checklist Template artifact data schema
CHECKLIST_TEMPLATE_DATA_SCHEMA = {
    "type": "object",
    "required": ["name", "items"],
    "properties": {
        "name": {
...
# Decision Validation artifact data schema (append-only history)
DECISION_VALIDATION_DATA_SCHEMA = {
    "type": "object",
    "required": ["decision_id", "status"],
    "properties": {
        "decision_id": {
            "type": "string",
            "description": "The decision being validated"
        },
        "status": {
            "type": "string",
        ...
        "confidence_delta": {
            "type": "number",
            "description": "Confidence change applied (+0.1, -0.2, etc.)"
        },
        "confidence_after": {
            "type": "number",
            "description": "Confidence value after this validation"
        },
        "episode_id": {
            "type": "string",
            "description": "Episode that provided evidence"
        },
        "notes": {
            "type": "string",
...
DATA_SCHEMAS = {
    ...
    "incident_rca": INCIDENT_RCA_DATA_SCHEMA,
    "recent_change": RECENT_CHANGE_DATA_SCHEMA,
    "design_reference": DESIGN_REFERENCE_DATA_SCHEMA,
    "checklist_template": CHECKLIST_TEMPLATE_DATA_SCHEMA,
    "decision_validation": DECISION_VALIDATION_DATA_SCHEMA
}
...
TYPE_DIRECTORIES = {
    ...
    "incident_rca": "incidents",
    "recent_change": "changes",
    "design_reference": "design_refs",
    "checklist_template": "checklists",
    "decision_validation": "decision_validations"
}
...
def validate_artifact(artifact: dict[str, Any]) -> tuple[bool, str]:
    ...
    # Check id format (allow shortened prefixes for some types)
    ...
    prefix_map = {
        ...
        "incident_rca": "inc_",
        "recent_change": "chg_",
        "design_reference": "dref_",
        "checklist_template": "chk_",
        "decision_validation": "dval_"
    }
    ...
    elif artifact_type == "incident_rca" and ("symptom" not in data or "actual_cause" not in data or "fix" not in data):
        return False, "Incident RCA must have 'symptom', 'actual_cause', and 'fix' fields"
    elif artifact_type == "recent_change" and ("scope" not in data or "change" not in data):
        return False, "Recent change must have 'scope' and 'change' fields"
    elif artifact_type == "design_reference" and ("product_name" not in data or "pattern" not in data):
        return False, "Design reference must have 'product_name' and 'pattern' fields"
    elif artifact_type == "checklist_template" and ("name" not in data or "items" not in data):
        return False, "Checklist template must have 'name' and 'items' fields"
