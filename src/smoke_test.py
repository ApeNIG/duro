...
def run_smoke_tests():
    ...
    success, msg, validation_id = store.validate_decision(
        decision_id=dec_id,
        status="validated",
        episode_id=ep_id5,
        result="success",
    ...
    if not success:
        ...
        log(f"Validated decision: {msg} (validation_id: {validation_id})")
    ...
    # =========================================================
    # TEST 7: Write -> Search Verification (Sync Embedding)
    # =========================================================
    print("\n--- TEST 7: Write -> Search Verification ---\n")

    import secrets
    from embeddings import embed_text, is_embedding_available

    # Generate unique tokens that won't match anything else
    test_tokens = [f"w2s_tok_{secrets.token_hex(6)}" for _ in range(10)]
    created_ids = []

    log(f"Creating 10 artifacts with unique tokens...")

    # Create 10 facts with unique tokens
    for i, token in enumerate(test_tokens):
        success, fact_id, _ = store.store_fact(
            claim=f"Write-search test {token} verifies sync embedding works correctly",
            confidence=0.5,  # Use 0.5 to avoid downgrade (no source_urls)
            tags=["write-search-test", token],
            sensitivity="internal"
        )
        if success:
            created_ids.append((fact_id, token))
        else:
            log(f"Failed to create fact {i}: {fact_id}", "FAIL")
            all_passed = False

    log(f"Created {len(created_ids)} artifacts")

    # Check embedding queue - should be 0 or near-0 if sync embedding works
    from embedding_worker import EmbeddingQueue
    queue = EmbeddingQueue(DEFAULT_MEMORY_DIR)
    pending_before_search = queue.get_pending_count()

    if pending_before_search > len(created_ids):
        log(f"Embedding queue has {pending_before_search} pending (expected near 0 for sync)", "WARN")
    else:
        log(f"Embedding queue: {pending_before_search} pending", "PASS")

    # Verify each artifact via FTS search
    fts_hits = 0
    for fact_id, token in created_ids:
        results = store.index.fts_search(token, limit=5)
        found = any(r["id"] == fact_id for r in results)
        if found:
            fts_hits += 1
        else:
            log(f"FTS miss for {fact_id} (token: {token})", "WARN")

    if fts_hits == len(created_ids):
        log(f"FTS: {fts_hits}/{len(created_ids)} hits", "PASS")
    else:
        log(f"FTS: {fts_hits}/{len(created_ids)} hits (expected all)", "FAIL")
        all_passed = False

    # Verify each artifact has embedding in database
    embeddings_found = 0
    with sqlite3.connect(DEFAULT_DB_PATH) as conn:
        for fact_id, _ in created_ids:
            cursor = conn.execute(
                "SELECT 1 FROM embedding_state WHERE artifact_id = ?",
                (fact_id,)
            )
            if cursor.fetchone():
                embeddings_found += 1

    if embeddings_found == len(created_ids):
        log(f"Embeddings: {embeddings_found}/{len(created_ids)} stored", "PASS")
    else:
        log(f"Embeddings: {embeddings_found}/{len(created_ids)} stored (expected all)", "FAIL")
        all_passed = False

    # Verify vector search returns results (if embeddings available)
    if is_embedding_available():
        vector_hits = 0
        for fact_id, token in created_ids[:3]:  # Test first 3 to save time
            # Generate embedding for query
            query_emb = embed_text(f"Write-search test {token}")
            if query_emb:
                vec_results = store.index.vector_search(query_emb, limit=10)
                found = any(r["id"] == fact_id for r in vec_results)
                if found:
                    vector_hits += 1

        if vector_hits >= 2:  # Allow some tolerance
            log(f"Vector search: {vector_hits}/3 hits in top-10", "PASS")
        else:
            log(f"Vector search: {vector_hits}/3 hits (expected at least 2)", "WARN")
    else:
        log("Vector search skipped (embeddings not available)", "INFO")

    # Verify index rows exist
    index_rows = 0
    with sqlite3.connect(DEFAULT_DB_PATH) as conn:
        for fact_id, _ in created_ids:
            cursor = conn.execute(
    ...
    if index_rows == len(created_ids):
        log(f"Index rows: {index_rows}/{len(created_ids)} present", "PASS")
    else:
        log(f"Index rows: {index_rows}/{len(created_ids)} present (expected all)", "FAIL")
        all_passed = False

    # Final queue check - should have drained
    pending_after = queue.get_pending_count()
    if pending_after <= pending_before_search:
        log(f"Queue stable/drained: {pending_after} pending", "PASS")
    else:
        log(f"Queue grew during test: {pending_before_search} → {pending_after}", "WARN")

    # =========================================================
    # TEST 8: Logging Creates Artifacts
    # =========================================================
    # This test prevents regression of the bug where store_log was never called
    print(f"\n--- TEST 8: Logging Creates Artifacts ---\n")

    test_token = f"smoke_log_test_{secrets.token_hex(4)}"
    log_success, log_id, log_path = store.store_log(
        event_type="smoke_test",
        message=f"Smoke test log entry {test_token}",
        tags=["smoke-test", "logging-verification"]
    )

    if log_success and log_id:
        log(f"store_log returned success: {log_id}", "PASS")
    else:
        log(f"store_log failed to create artifact", "FAIL")
        all_passed = False

    # Verify artifact exists in index
    if log_success:
        with sqlite3.connect(DEFAULT_DB_PATH) as conn:
            cursor = conn.execute(
                "SELECT 1 FROM artifacts WHERE id = ?",
            ...
            if cursor.fetchone():
                log(f"Log artifact indexed in DB", "PASS")
            else:
                log(f"Log artifact NOT found in index (store_log bug!)", "FAIL")
                all_passed = False

        # Verify file exists
        if Path(log_path).exists():
            log(f"Log artifact file exists", "PASS")
        else:
            log(f"Log artifact file missing: {log_path}", "FAIL")
            all_passed = False
